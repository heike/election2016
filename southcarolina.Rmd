...
#    mpolls$voterType <- factor(mpolls$voterType, c("RV", "LV", "A"))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mpolls, (year >=2015 & month >=10) | (year >= 2016))) +
...
mpolls <- subset(mpolls, Poll != "RCP Average")
 ...
 lastPoll <- sprintf("Last Poll on %s", max(mpolls$endDate))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mpolls, (year==2016) | (year >=2015 & month >=10))) +
      caucuslayer(max(mpolls$value), "D") +
      geom_smooth(aes(group=variable), colour="grey30", se=FALSE, size=.75) +
