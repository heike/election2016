...
polls[1,]$endDate <- as.Date("2016/02/01")
...
#    mpolls$voterType <- factor(mpolls$voterType, c("RV", "LV", "A"))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mpolls, (Poll != "Final Results") & (year >=2015 & month >=7) | (year == 2016))) +
...
mpolls <- subset(mpolls, Poll != "RCP Average")
 ...
 lastPoll <- sprintf("Last Poll on %s", max(mpolls$endDate))
    cols <- c(brewer.pal(12, name="Paired"), "grey30", "grey50", "grey70", "grey80", "grey60", "black", "black", "black")
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mpolls, (year >=2015 & month >=7) | (year == 2016))) +
      ...
      scale_shape_manual("Type of Poll",values=c(19, 1, 4)) +
      ggtitle(lastPoll) + 
      geom_point(data=subset(mpolls,Poll=="Final Results"), pch=1, size=4)
