...
Polling results in percentage points for all Republican Presidential candidates. Polls from major pollsters are shown (by color), the line is a non-parametric loess smooth to show the trend over time. Only polls since the beginning of July 2015 are included.
...
reps$Poll <- fixName(fixName(reps$Poll))
...
lastPoll <- sprintf("Last Poll on %s", max(mreps$endDate))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mreps, year >=2015 & month >= 7)) +
