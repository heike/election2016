...
Polling results in percentage points for all Republican Presidential candidates. Polls from major pollsters are shown (by color), the line is a non-parametric loess smooth to show the trend over time. Only polls since the end of 2015 are included.
...
mreps <- melt(reps, measure.vars=4:11, na.rm=TRUE)
...
lastPoll <- sprintf("Last Poll on %s", max(mreps$endDate))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mreps, (year >=2016) | (year >=2015 & month >= 11))) +
