---
title: "Republican Presidential Candidates"
author: "Heike Hofmann"
date: "September 24, 2015"
output: html_document
---

Polling results in percentage points for all Republican Presidential candidates. Polls from major pollsters are shown (by color), the line is a non-parametric loess smooth to show the trend over time. Only polls since the beginning of the year are included.

```{r, echo=FALSE, fig.width=10, fig.height=10, message=FALSE}
library(XML)
library(lubridate)
library(plyr)
library(reshape2)
library(ggplot2)
library(RColorBrewer)

###############
#  functions
sub <- function(x, replace=c("--"), by=NA) {
  x <- as.character(x)
  idx <- which(x %in% replace)
  if (length(idx) > 0) x[idx] <- by
  x
}

############
rcp.rep <- "http://www.realclearpolitics.com/epolls/2016/president/us/2016_republican_presidential_nomination-3823.html"
tables <- readHTMLTable(rcp.rep)
reps <- data.frame(tables[[6]])
reps <- reps[nrow(reps):1,]

repsNA <- lapply(reps, sub)
write.csv(repsNA, "temp.csv", row.names=FALSE)
reps <- read.csv("temp.csv")
reps$year <- 2015
reps$year[1] <- 2012
reps$year[2:19] <- 2013
reps$year[20:46] <- 2014

dates <- strsplit(as.character(reps$Date), split=" - ")
d2 <- ldply(dates, function(x) data.frame(
  startDate = mdy(paste(x[1],"15", sep="/")),
  endDate = mdy(paste(x[2],"15", sep="/"))))
year(d2$startDate) <- reps$year
year(d2$endDate) <- reps$year

reps <- data.frame(reps, d2)

mreps <- melt(reps, measure.vars=3:17, na.rm=TRUE)
mreps <- subset(mreps, Poll != "RCP Average")


cols <- c(brewer.pal(12, name="Paired"), "grey30")
qplot(x=startDate, xend=endDate, yend=value, y=value, colour=Poll, 
      facets=~variable, size=I(1.5), data=subset(mreps, year >=2015), geom="segment") +
  geom_point(aes(x=endDate, y=value), size=2.5) + 
  geom_smooth(aes(group=variable), colour="grey30", se=FALSE, size=.75) +
  scale_colour_manual(values=cols) + theme_bw() +
  theme(legend.position="bottom") + xlab("Date of Poll") + ylab("Percent")

```