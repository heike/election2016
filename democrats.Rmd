...
mdems <- melt(dems, measure.vars=5:6, na.rm=TRUE)
mdems <- subset(mdems, Poll != "RCP Average")
 ...
 lastPoll <- sprintf("Last Poll on %s", max(mdems$endDate))
    ...
    p <- ggplot(aes(x=endDate, y=value, colour=Poll), data=subset(mdems, (year >=2015 & month >=11) | (year >=2016))) +
      caucuslayer(max(mdems$value), "D") +
