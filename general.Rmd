...
plotCandidates <- function(rcp.gen) {
  ...
  dims <- plyr::ldply(tables, function(x) if (is.null(x)) c(0,0) else dim(x))
...
#  gen <- gen[nrow(gen):1,]
  ...
  year(d2$startDate) <- fixYear(d2$startDate)
  year(d2$endDate) <- fixYear(d2$endDate)
  d2$year <- year(d2$endDate)
  ...
  democrat <- gsub("..D.","", democrat, fixed=TRUE)
  republican <- gsub("..R.","", republican, fixed=TRUE)
  
  fname <- sprintf("general-%s-%s.csv",democrat, republican)
  write.csv(gen, fname, row.names=FALSE)
  ...
  lastPoll <- sprintf("Last Poll on %s", max(gen$endDate))
...
plotCandidates("http://www.realclearpolitics.com/epolls/2016/president/us/general_election_cruz_vs_clinton-4034.html")

plotCandidates("http://www.realclearpolitics.com/epolls/2016/president/us/general_election_cruz_vs_sanders-5742.html")

plotCandidates("http://www.realclearpolitics.com/epolls/2016/president/us/general_election_rubio_vs_clinton-3767.html")

plotCandidates("http://www.realclearpolitics.com/epolls/2016/president/us/general_election_rubio_vs_sanders-5564.html")
